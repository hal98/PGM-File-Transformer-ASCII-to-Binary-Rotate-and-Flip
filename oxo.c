...
bool position(Board *b, char *text, Position *p) {
  if (strcmp("a1", text) == 0 && b->cells[0][0] == B){
    p->row=0; p->col=0; return true;
  }
  if (strcmp("a2", text) == 0 && b->cells[0][1] == B){
    p->row=0; p->col=1; return true;
  }
  if (strcmp("a3", text) == 0 && b->cells[0][2] == B){
    p->row=0; p->col=2; return true;
  }
  if (strcmp("b1", text) == 0 && b->cells[1][0] == B){
    p->row=1; p->col=0; return true;
  }
  if (strcmp("b2", text) == 0 && b->cells[1][1] == B){
    p->row=1; p->col=1; return true;
  }
  if (strcmp("b3", text) == 0 && b->cells[1][2] == B){
    p->row=1; p->col=2; return true;
  }
  if (strcmp("c1", text) == 0 && b->cells[2][0] == B){
    p->row=2; p->col=0; return true;
  }
  if (strcmp("c2", text) == 0 && b->cells[2][1] == B){
    p->row=2; p->col=1; return true;
  }
  if (strcmp("c3", text) == 0 && b->cells[2][2] == B){
    p->row=2; p->col=2; return true;
  }
  else return false;
}
...
void move(Board *b, Position *p) {
  ...
  if (b->player > 80) b->player = b->player -9;
  else b->player = b->player +9;
  b->moves = b->moves + 1;
}
...
bool line(char x, char y, char z) {
  if (x==X && y==X && z==X) return true;
  else if (x==O && y==O && z==O) return true;
  else return false;
}
...
bool won(Board *b) {
  ...
  else if (line(b->cells[1][0],b->cells[1][1],b->cells[1][2])) return true;
  else if (line(b->cells[2][0],b->cells[2][1],b->cells[2][2])) return true;
  else if (line(b->cells[0][0],b->cells[1][0],b->cells[2][0])) return true;
  else if (line(b->cells[0][1],b->cells[1][1],b->cells[2][1])) return true;
  else if (line(b->cells[0][2],b->cells[1][2],b->cells[2][2])) return true;
  else if (line(b->cells[0][0],b->cells[1][1],b->cells[2][2])) return true;
  else if (line(b->cells[2][0],b->cells[1][1],b->cells[0][2])) return true;
  else return false;
}
...
bool drawn(Board *b) {
  if (!won(b) && b->moves == 9) return true;
  else return false;
}
